...
class QC_Zone:
    ...
    def _get_zone_transition_counts(self):
        """
        Count transitions between zones during the workout.
        Returns a DataFrame with counts of transitions from each zone to every other zone.
        """
        if self.hr is None or self.hr.empty:
            self.err["zone_transitions"] = ["hr data missing for zone transition QC", None]
            return None
        hr_df = self.hr.copy()
        hr_df["time"] = pd.to_datetime(hr_df["time"])
        hr_df = hr_df.sort_values("time").reset_index(drop=True)
        ...
        # Count transitions
        hr_df["prev_zone"] = hr_df["zone"].shift(1)
        transitions = hr_df[hr_df["zone"] != hr_df["prev_zone"]].dropna(subset=["prev_zone"])
